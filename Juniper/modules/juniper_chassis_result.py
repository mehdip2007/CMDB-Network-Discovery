...
pic_port_info = re.compile(r"\s\s\d+\s\s.*\S+")


def online_to_csv(data, slot, ip):
    online_files = open("online-{}-fpc-{slot}-pic-{slot}.csv".format(ip, slot=slot), "w")
    online_files.write("IP,Port,Cable type,type,Xcvr vendor,part number,length,Firmware" + '\n')
    matches = pic_port_info.findall(data)
    if matches:
        for match in matches:
            ...
            for each in all_data:
                ...
                online_files.write(",".join(ip + result) + '\n')
