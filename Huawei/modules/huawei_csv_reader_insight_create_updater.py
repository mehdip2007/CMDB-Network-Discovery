...
from .huawei_ipbb_insight_mapper import attributes_map
import json
...
def update_insight_object(data, obj_id):
    fields = {"objectTypeId": config.get('huawei_ipbb_type_id'), "attributes": []}
    ...
    if response.status_code != 201:
        print(f"{response.status_code} for {data.get('router_name')}")
...
def search_in_insight(data):
    ...
    params = {
        "objectSchemaId": 1,
        "iql": f'ObjectTypeId={config.get("huawei_ipbb_type_id")} AND "Name" = "{router_name}" AND "IP Address" = "{router_ip}"',
        "resultPerPage": 5
    }
    ...
    if len(json_response['objectEntries']) == 0:
        ...
        print(f"******** {router_name} and {router_ip} has multiple record ********")
        raise Exception
