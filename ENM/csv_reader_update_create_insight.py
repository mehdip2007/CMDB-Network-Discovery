...
def update_insight_object(data, obj_id):
    ...
    try:
        response = requests.put(
            'http://10.132.58.167/rest/insight/1.0/' + f"object/{obj_id}/",
...
def create_insight_object(data):
    ...
    try:
        response = requests.post(
            'http://10.132.58.167/rest/insight/1.0/' + "object/create/",
            auth=(config.get("insight_username"), config.get("insight_password")),
            data=json.dumps(fields),
            headers=headers,
            verify=False,
...
def search_in_insight(data):
    ...
    params = {
        "objectSchemaId": 1,
        "iql": f'ObjectTypeId=104 AND "Serial Number" = "{serial_number}" AND "Inventory Unit" = "{inventory_unit}"',
        "resultPerPage": 5
    }
    enm_insight_iql_link = f'http://10.132.58.167/rest/insight/1.0/iql/objects?'
    responses = requests.get(enm_insight_iql_link,
